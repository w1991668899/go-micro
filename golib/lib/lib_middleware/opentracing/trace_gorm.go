...
const (
	GORMParentSpan = "opentracingParentSpan"
	GORMChildSPAN  = "gorm_span"
)

func SetGORMCallbacks(db *gorm.DB) {
...
func GetDbWithSpanFromContext(ctx context.Context, db *gorm.DB) *gorm.DB {
	if ctx == nil {
		return db
	}
	...
	return db.Set(GORMParentSpan, parentSpan)
}
...
func (c *callbacks) before(scope *gorm.Scope) {
	val, ok := scope.Get(GORMParentSpan)
	if !ok {
		return
	}
	...
	scope.Set(GORMChildSPAN, sp)
}

func (c *callbacks) after(scope *gorm.Scope, operation string) {
	val, ok := scope.Get(GORMChildSPAN)
	if !ok {
		return
	}
	...
	sp.SetTag("sql.vars", scope.SQLVars)
...
// doc see http://gorm.io/docs/write_plugins.html
